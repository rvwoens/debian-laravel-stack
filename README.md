...
[![GitHub Release](https://img.shields.io/badge/release-2.0.00-blue)](https://github.com/rvwoens/Debian-laravel-stack)
...
```bash
curl -s -L https://github.com/rvwoens/debian-laravel-stack/archive/v2.0.00.tar.gz | tar -xz
cd debian-laravel-stack-2.0.00
./setup_full
```
...
- creates a nginx config in ```/etc/nginx/sites-available``` and enables it, optionally including Certbot SSL setup
...
- You can also create a ```after_deploy_is_live``` script that runs AFTER the swap. Typically used to restart queue services and whatnot.

If any of the steps failed ```puller``` aborts before making the release current. So your live version is safe.

Puller is normally called by ```pusher``` from your local dev machine, but you can also do this your own way:
Call ```puller``` from your local development machine for further integration:
...
* asks for ssh public key to access this account remotely. Paste the contents of your ~/.ssh/id_ed25519.pub
...
#### 10 nginx
set up nginx for php-fpm and prepare for multiple virtual hosts with the sites-enabled/sites-available pattern and install Certbot
